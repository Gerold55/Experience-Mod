...
minetest.override_item("moreores:mineral_tin", {
	groups = {cracky=3, xp=1},
})
minetest.override_item("moreores:mineral_silver", {
	groups = {cracky=3, xp=2},
})
minetest.override_item("moreores:mineral_mithril", {
	groups = {cracky=3, xp=3},
})
end

if minetest.get_modpath("technic") ~= nil then
minetest.override_item( "technic:mineral_uranium", {
	groups = {cracky=3, radioactive=1000,xp=3},
}) 

minetest.override_item( "technic:mineral_chromium", {
	groups = {cracky=3,xp=2},
}) 
...
local gameTime = minetest.get_gametime()
	...
	for _,player in ipairs(minetest.get_connected_players()) do
		...
		xptemp:close()
				...
				if (playerInfo["experiencetotal"]) ~= nil then
					playerInfo["experiencebar"] = playerInfo["experiencetotal"] - 0
					
					playerInfo["textnumber"] = 0
				
				    while playerInfo["experiencebar"] >= MAX_HUD_EXPERIENCE do
					  playerInfo["textnumber"]= playerInfo["textnumber"] + 1
					  playerInfo["experiencebar"] = playerInfo["experiencebar"] - MAX_HUD_EXPERIENCE
				    end
				end
				...
				player:hud_change(playerInfo["hud"], "number", playerInfo["experiencebar"])
